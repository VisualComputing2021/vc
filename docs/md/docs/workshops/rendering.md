...
<p style="text-align: justify">Sabemos que la ecuación de una linea recta es: y=mx+b dónde m es la pendiente de la recta que podemos calcular como y{final}-y{inicial}/x{final}-{yinicial}. Basado en esto el algoritmo DDA calcula para cada segmento de linea la siguientes ecuaciones xi = xi-1 + 1 y yi = yi-1 + m. Facilmente podemos ver como el algoritmo tiene en cuenta su valor anterior para calcular los siguientes valores.</p> 
